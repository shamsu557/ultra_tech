...
app.post("/api/student/apply", (req, res) => {
  const { firstName, lastName, email, phone, gender, dateOfBirth, address, courseId, schedule } = req.body

  // Generate application number
  const applicationNumber = "APP" + Date.now()

  const query = `INSERT INTO students (application_number, first_name, last_name, email, phone, gender, date_of_birth, address, course_id, status) 
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Applied')`

  db.query(
    query,
    [applicationNumber, firstName, lastName, email, phone, gender, dateOfBirth, address, courseId],
    (err, result) => {
      if (err) {
...
app.post("/api/student/setup-security", async (req, res) => {
  ...
  try {
    ...
    db.query(query, [hashedPassword, securityQuestion, securityAnswer], (err, result) => {
      if (err) {
...
app.post("/api/payment/verify", (req, res) => {
  const { reference, studentId, paymentType, installmentType } = req.body

  // In a real implementation, you would verify with Paystack API
  // For now, we'll simulate successful payment

  const amount = paymentType === "Application" ? 200 : installmentType === "full" ? 500 : 250
